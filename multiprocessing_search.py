...
def multiprocess_search(file_list, keywords, num_processes=None):
    """
    Основна функція для багатопроцесорного пошуку ключових слів у файлах.
    Розподіляє файли між процесами, збирає результати та повертає фінальний словник.
    """
    result_queue = Queue()  # Черга для збору результатів
    processes = []
    num_processes = min(num_processes or cpu_count(), len(file_list))  # Обмеження кількості процесів
    chunk_size = len(file_list) // num_processes  # Розмір частини файлів для кожного процесу

    # Розподіл файлів між процесами
    for i in range(num_processes):
        start_idx = i * chunk_size
        end_idx = (i + 1) * chunk_size if i != num_processes - 1 else len(file_list)
        process = Process(
...
if __name__ == "__main__":
    ...
    results = multiprocess_search(file_paths, keywords, num_processes=4)
